...
public class MultiThreadedServer {
    ...
    public static void main(String[] args) {
        ...
        MultiThreadedServer multi = new MultiThreadedServer(1337);
        multi.serveClients();
    ...
    private class ReceptionistWorker implements Runnable {
        ...
        private class ServantWorker implements Runnable {
            ...
            public void run() {
                ...
                out.println("Please respect the following format : CMD <arg1> <arg2>");
                out.flush();
                try {
                    LOG.info("Reading until client sends BYE or closes the connection...");
                    while ((shouldRun) && (line = in.readLine()) != null) {
                        String[] lineSplit = line.split(" ");
                        if (lineSplit[0].equalsIgnoreCase("bye")) {
                            shouldRun = false;
                        }
                        switch(lineSplit[0].toLowerCase()){
                            case "add" :
                                out.println("Result : " + String.valueOf(Double.valueOf(lineSplit[1]) + Double.valueOf(lineSplit[2])));
                                break;
                            case "mul" :
                                out.println("Result : " + String.valueOf(Double.valueOf(lineSplit[1]) * Double.valueOf(lineSplit[2])));
                                break;
